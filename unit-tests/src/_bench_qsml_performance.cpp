...
public:
    ...
    void bench_reductions() {
        ...
        std::vector<std::tuple<size_t, size_t, size_t>> shapes = {
            ...
            {256, 256, 8},        // 3D: 256x256x8 (reduced from 16)
            {128, 128, 16},       // 3D: 128x128x16 (reduced from 64)
        };
...
int main(int argc, char** argv) {
    try {
        ...
        if (argc > 1 && std::string(argv[1]) == "--pool-only") {
            bench.bench_memory_pool();
            bench.bench_memory_pool_vs_operations();
            bench.print_results();
        } else {
            bench.run_all();
        }
